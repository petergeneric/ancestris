...
public class MergeGedcomTool {
    ...
    public MergeGedcomTool(tmpMerge report, Log log) {
        this.report = report;
        this.log = log;
    }
    ...
    public boolean run(Gedcom gedcom) {
        ...
        if (!setting_chkdup) {
            ...
            log.timeStamp(6, report.translate("logStart") + ": ");
            if (!makeUniqueIds(gedcomA, gedcomB, idNewOld)) {
                return false;
            }
            log.timeStamp(6, report.translate("logEnd") + ": ");
            log.write(" ");
    ...
    private Gedcom getGedcomFromUser(String msg) {
        ...
        // Get file from user
        File fileX = getFileFromUser(msg);
        if (fileX == null) {
            return null;
        }
    ...
    static private int extractNumber(String str) {
        ...
        if (end == start) {
            ...
            return (int) Integer.parseInt(str.substring(start, end));
        }
    }
    ...
    private boolean prepareSets(Gedcom gedcomX, Gedcom gedcomY, Map typeEntsX, Map typeEntsY, double[] sizes) {
        ...
        for (int i = 0; i < typeEnt.length; i++) {
            if (i == 1) {
                continue; // do not try to match families directly
            }
        ...
        for (int i = 0; i < typeEnt.length; i++) {
            ...
            if (i == 0) {
                if (report.setting_analysis1 == ANA1_SUBSET) {
                    indiSel = (Indi) getEntityFromUser(report.translate("user_individual"), gedcomX, Gedcom.INDI);
                    if (indiSel != null) {
                        ...
                        String sel = ""; //(String) getValueFromUser(report.translate("user_choose_subset"), choices, choices[0]);
                        if (sel != null) {
                            log.write(0, 6, "", 0, sel);
                            n = ((sel == choices[0]) ? 0 : 1);
    ...
    private boolean trimSpans(List personX, List personY) {
        ...
        for (Iterator it = personX.iterator(); it.hasNext();) {
            Person p = (Person) it.next();
            if (PersonFactory.compareSpans(p, spanMaxX) > 0) {
                PersonFactory.copy(p, spanMaxX);
            }
            if (PersonFactory.compareSpans(p, spanMinX) < 0) {
                PersonFactory.copy(p, spanMinX);
            }
        }
        ...
        for (Iterator it = personY.iterator(); it.hasNext();) {
            Person p = (Person) it.next();
            if (PersonFactory.compareSpans(p, spanMaxY) > 0) {
                PersonFactory.copy(p, spanMaxY);
            }
            if (PersonFactory.compareSpans(p, spanMinY) < 0) {
                PersonFactory.copy(p, spanMinY);
            }
        }
    ...
    private boolean assessMatches(Gedcom gedcomX, Gedcom gedcomY, Map typeEntsX, Map typeEntsY, Map confList, Map overlaps, Map scoreStats, Map idMap, ProgressStatus progress, boolean duplicates, double size, Map idNewOld) {
        ...
        if (!setting_appendFiles) {
            ...
            for (int i = 0; i < typeEnt.length; i++) {
                ...
                if (i == 0) {
                    if (duplicates) {
                        if (!runAlgoDupIndi(listX, listY, confList, progress, idNewOld)) {
                            return false;
                        }
                    } else {
                        if (!runAlgoMatchIndi(listX, listY, confList, progress, idNewOld)) {
                            return false;
                        }
                    }
                } else {
                    if (duplicates) {
                        if (!runAlgoDupInfo(listX, listY, confList, progress, idNewOld)) {
                            return false;
                        }
                    } else {
                        if (!runAlgoMatchInfo(listX, listY, confList, progress, idNewOld)) {
                            return false;
                        }
                    }
                }
        ...
        // Consolidate
        if (report.setting_keepAecl) {
            ZfromX.addAll(Xexclusive);
        }
        if (report.setting_keepAcon) {
            ZfromX.addAll(Xconnected);
        }
        if (report.setting_keepBcon) {
            ZfromY.addAll(Yconnected);
        }
        if (report.setting_keepBecl) {
            ZfromY.addAll(Yexclusive);
        }
    ...
    private boolean runAlgoDupIndi(List listX, List listY, Map confList, ProgressStatus progress, Map idNewOld) {
        ...
        for (Iterator it1 = listX.iterator(); it1.hasNext();) {
            ...
            for (Iterator it2 = listY.iterator(); it2.hasNext();) {
                ...
                if (p2.yearMin > p1.yearMax) {
                    break;
                }
                if (p2.yearMax < p1.yearMin) {
                    continue;
                }
                if ((p1.sex != p2.sex) && (p1.sex != UNKNOWN) && (p2.sex != UNKNOWN)) {
                    continue;
                }
                key = p1.id + "x" + p2.id;
                match = (ConfidenceMatch) confList.get(key);
    ...
    private boolean displayMatches(Map matches, Map overlaps, Map scoreStats, boolean duplicates) {
        ...
        if (!confList.isEmpty()) {
            ...
            if (i == max) {
                log.write(0, 6, "", 0, report.translate("logDispOther") + ".");
            }
            log.write(" ");
        }
...
//log.write("scanX wife = "+wifeX+" - matchWife="+matchWifeX);
            ...
            for (Iterator ity = listFamsY.iterator(); ity.hasNext();) {
                ...
                if (famX.getId().equals(famY.getId())) {
                    continue;   // do not match same families
                }
                ConfidenceMatch matchTmp = (ConfidenceMatch) matches.get(famY.getId() + "x" + famX.getId());
                if (matchTmp != null) {
                    continue;   // do not match if reverse already matched
                }
...
//log.write("match found: famY = "+famY);
                    ...
                    if (match.id2 == null && duplicates) {
                        match.id2 = (String) (match.ent2.getId());
                    }
    ...
    private int[] mapPlaceFormat(Gedcom gedcomX, Gedcom gedcomY, int headerChosen) {
        ...
        for (int i = 0; i < tags1.length; i++) {
            ...
            String selection = null; //(String) report.getValueFromUser(msg, (Object[]) tags2.toArray(), tags2.get(0));
            int iSel = 0;
            if (selection == null) {
    ...
    private Comparator sortEntities = new Comparator() {
        ...
        public int compare(Object o1, Object o2) {
            ...
            if (tag1 == Gedcom.INDI) {
                s1 = "A";
            } else if (tag1 == Gedcom.FAM) {
                s1 = "B";
            } else if (tag1 == Gedcom.NOTE) {
                s1 = "C";
            } else if (tag1 == Gedcom.SOUR) {
                s1 = "D";
            } else if (tag1 == Gedcom.REPO) {
                s1 = "E";
            } else if (tag1 == Gedcom.SUBM) {
                s1 = "F";
            }
            // tag2
            if (tag2 == Gedcom.INDI) {
                s2 = "A";
            } else if (tag2 == Gedcom.FAM) {
                s2 = "B";
            } else if (tag2 == Gedcom.NOTE) {
                s2 = "C";
            } else if (tag2 == Gedcom.SOUR) {
                s2 = "D";
            } else if (tag2 == Gedcom.REPO) {
                s2 = "E";
            } else if (tag2 == Gedcom.SUBM) {
                s2 = "F";
            }
            ...
            while (start <= end && !Character.isDigit(id1.charAt(start))) {
                start++;
            }
            while (end >= start && !Character.isDigit(id1.charAt(end))) {
                end--;
            }
            if (end < start) {
                n1 = 0;
            } else {
                n1 = (int) Integer.parseInt(id1.substring(start, end + 1));
            }

            start = 0;
            end = id2.length() - 1;
            while (start <= end && !Character.isDigit(id2.charAt(start))) {
                start++;
            }
            while (end >= start && !Character.isDigit(id2.charAt(end))) {
                end--;
            }
            if (end < start) {
                n2 = 0;
            } else {
                n2 = (int) Integer.parseInt(id2.substring(start, end + 1));
            }
...
//if (debug) log.write("     --- assessRelatives - score = "+score);
    ...
    private ConfidenceMatch assessConfidenceInfo(Info i1, Info i2, boolean duplicates, Map confList, Map idNewOld) {
        ...
        if (i1.titleLength > 0 && i2.titleLength > 0) {
            if (i1.title.compareTo(i2.title) == 0) {
                ...
                score = matchCode(i1.titleCode, i1.titleLength, i2.titleCode, i2.titleLength);
            }
            if (score < 90) {
                scoreTitle = 1;
            } else {
                scoreTitle = ((double) (score * 10)) / 100;
            }
        }
        ...
        if (i1.authLength > 0 && i2.authLength > 0) {
            if (i1.auth.compareTo(i2.auth) == 0) {
                ...
                score = matchCode(i1.authCode, i1.authLength, i2.authCode, i2.authLength);
            }
            if (score < 90) {
                scoreAuth = 1;
            } else {
                scoreAuth = ((double) (score * 5)) / 100;
            }
        }
        ...
        if (i1.abbrLength > 0 && i2.abbrLength > 0) {
            if (i1.abbr.compareTo(i2.abbr) == 0) {
                ...
                score = matchCode(i1.abbrCode, i1.abbrLength, i2.abbrCode, i2.abbrLength);
            }
            if (score < 90) {
                scoreAbbr = 1;
            } else {
                scoreAbbr = ((double) (score * 5)) / 100;
            }
        }
        ...
        if (i1.textLength > 0 && i2.textLength > 0) {
            if (i1.text.compareTo(i2.text) == 0) {
                ...
                score = matchText(i1.textCode, i2.textCode);
            }
            if (score < 70) {
                scoreText = 1;
            } else {
                scoreText = ((double) (score * 600)) / 100;
            }
        }
    ...
    private void displayConfList(Map confList) {
        ...
        ConfidenceMatch match = null;
        log.write(0, 0, "", 0, "key:__x__" + "\t" + "id1" + "\t" + "id2" + "\t" + "strid2" + "\t" + "lvl" + "\t" + "conf" + "\t" + "tbm" + "\t" + "choice");
        for (Iterator it = confList.keySet().iterator(); it.hasNext();) {
            String key = (String) it.next();
            match = (ConfidenceMatch) confList.get(key);
            log.write(0, 0, "", 0, "key:" + key + "\t" + match.ent1.getId() + "\t" + match.ent2.getId() + "\t" + match.id2 + "\t" + match.confLevel + "\t" + match.confirmed + "\t" + match.toBeMerged + "\t" + match.choice);
        }
        return;
    }
    ...
    public final Entity getEntityFromUser(String msg, Gedcom gedcom, String tag) {
        ...
        // show it
        //showOpenDialog("select." + tag, "Merge", 0, new JComponent[]{new JLabel(msg), select}, "ok", null);
