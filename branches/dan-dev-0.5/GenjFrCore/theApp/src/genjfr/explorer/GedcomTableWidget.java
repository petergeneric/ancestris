 ...
 */
/*package*/ public class GedcomTableWidget extends JTable implements ContextProvider {
  ...
  private static final Object COLUMNS[] = {
    Resources.get("genj.app").getString("cc.column_header.name"),
...
//  }
  ...
  public Context getSelectedContext() {
    ...
    public void selectionChanged(Workbench workbench, Context context, boolean actionPerformed) {
        GedcomDirectory.getInstance().selectionChanged(context);
    }
  ...
  private class GedcomTableModel extends AbstractTableModel implements GedcomDirectory.Listener, GedcomMetaListener {
    ...
    public Object getValueAt(int row, int col) {
      Context gedcom = GedcomDirectory.getInstance().getGedcoms().get(row);
      switch (col) {
        case 0: return gedcom.getGedcom().getName() + (gedcom.getGedcom().hasChanged() ? "*" : "" );
        case 8: return gedcom.getGedcom().getLastChange();
        default: return new Integer(gedcom.getGedcom().getEntities(Gedcom.ENTITIES[col-1]).size());
      }
    }
