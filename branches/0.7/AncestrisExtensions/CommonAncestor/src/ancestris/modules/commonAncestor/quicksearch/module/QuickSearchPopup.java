...
public class QuickSearchPopup extends javax.swing.JPanel 
     ...
     * @param maxResult nombre de resultats maximum affichés par defaut
    ...
    void selectNext() {
        ...
        } else if ( oldSel == -1 && jList1.getModel().getSize() > 0) {
            // modif michel
    ...
    void selectPrev() {
        ...
        if (oldSel > 0) {
            jList1.setSelectedIndex(oldSel - 1);
        } if ( oldSel == -1 && jList1.getModel().getSize() > 0) {
            // modif michel
...
//        result.setLocation(location);
    ...
    private boolean updateStatusPanel (boolean isInProgress) {
        ...
        boolean areNoResults = searchedNotEmpty && rModel.getSize() <= 0 && !isInProgress && !resultFound ;
