...
public class ReportToolBox extends Report {
     ...
     * entity types
     */
    private final static int ENT_ALL = 0,
            ENT_INDI = 1,
            ENT_FAM = 2,
            ENT_NOTE = 3,
            ENT_SOUR = 4,
    ...
    /**
     * While we generate information on stdout it's not really necessary because
     * we're returning a Document that is handled by the UI anyways
     */
    ...
    private void start(Gedcom gedcom, Indi indiDeCujus) {
        ...
        if (logOption && (log == null || !log.writeFile)) {
            return;
        }
        ...
        if (ret) {
            log.write(translate("Completed"));
        } else {
            log.write(translate("Error"));
        }
            ...
            try {
                gedcom.doUnitOfWork(new UnitOfWork() {
                    public void perform(Gedcom gedcom) throws GedcomException {
                        ...
                        if (logOption) {
                            log.write(translate("DetailsProcessed"));
                        }
                        log.write(" ");
                ...
                if (in != null && out != null) {
                    ...
                    while ((bytes_read = in.read(bufferIn)) != -1) {
                        for (i = 0; i < bytes_read; i++) {
                            if (bufferIn[i] != System.getProperty("line.separator").charAt(0)) {
                                continue;
                            }
                            recCount++;
     ...
     *
     * Principle 1 : only update age if birth date fully determined, unless
     * forced Principle 2 : no age for an EVEN after death (even if forced)
     *
     */
            ...
            gedcom.doUnitOfWork(new UnitOfWork() {
                public void perform(Gedcom gedcom) throws GedcomException {
                    ...
                    for (Iterator it = listEntities.iterator(); it.hasNext();) {
                        ...
                        if (!settings.geneForce && !preciseBirth(indi)) {
                            continue;
                        }
    ...
    private void updateAge(Indi indi, String eventTag, boolean force, boolean refresh) {
        ...
        if (evenProp != null) {
            ...
            if (prop != null && prop instanceof PropertyDate) {
                ...
                if (deathDate != null && deathDate.isValid() && pDate.compareTo(deathDate) > 0) {
                    afterDeath = true;
                }
            }
    ...
    private void updateAge(Fam fam, String eventTag, boolean force, boolean refresh) {
        ...
        if (evenProp != null) {
            ...
            if (prop != null && prop instanceof PropertyDate) {
                ...
                if (deathDateHusband != null && deathDateHusband.isValid() && pDate.compareTo(deathDateHusband) > 0) {
                    afterDeathHusband = true;
                }
                PropertyDate deathDateWife = wife == null ? null : wife.getDeathDate();
                if (deathDateWife != null && deathDateWife.isValid() && pDate.compareTo(deathDateWife) > 0) {
                    afterDeathWife = true;
                }
            }
    ...
    private boolean isToBeExcluded(String surname, String firstname, String parameter) {
        ...
        for (int i = 0; i < bits.length; i++) {
            ...
            if (surname.indexOf(bits[i]) > -1 || firstname.indexOf(bits[i]) > -1) {
                return true;
            }
        }
        return false;
