...
public class ReportLinesCirc extends Report {
    ...
    private void putperson(Fam family, Indi person, int level, int ahnen, int info, int dateformat) {
        ...
        if (eq(level, 1)) {
            ...
            numindilines += pCell.add(put_full_name(person, 0, 1, getel(levellength, level))
                    + getDateString(birth(person), death(person), dateformat), numindilines);
            writer.print(pCell.get(ahnen, info));
        }
        if (printmarr) {
    ...
    Indi father(Indi person) {
        return (person == null) ? null : husband(parents(person));
    }

    Indi mother(Indi person) {
        return (person == null) ? null : wife(parents(person));
    }

    String sex(Indi indi) {
        return ((indi.getSex() == PropertySex.MALE) ? "M" : "F");
    }
    /*
    ...
    private static class PersonCell {
        ...
        String get(int ahnen, int info) {
            ...
            for (offset = 1; offset <= max; offset++) {
                result += (cells.get(offset - 1)) + ") " + ahnen + " " + offset + " " + info + " " + max + "} addind\n";
            }
            return (result);
        }
        ...
        String getCenter(int sex) {
            ...
            int offset;
            for (offset = 1; offset <= max; offset++) {
                result += (cells.get(offset - 1)) + ") " + sex + " " + offset + " 0} addcenterindi\n";
            }
            return (result);
        }
    }
