...
/**
 * Class for encapsulating a path of tags that describe the way through a tree
 * of properties. An example for a path is TagPath("INDI:BIRT:DATE") which
 * denotes the <em>date</em> of property <em>birth</em> of an individual.
 *
 * @author Nils Meier
...
public class TagPath implements Comparable {
    ...
    /**
     * Returns a TagPath instance representing the specified String value. If a
     * new TagPath instance is not required, this method should generally be
     * used in preference to the constructor TagPath.TagPath(String), as this
     ...
     * Constructor for TagPath
     *
     * @param other tag from one to create new one
     */
     ...
     *
     * @param other tag from one to create new one
     * @param length to cut the new tag
     */
    ...
    public TagPath(TagPath other, String tag) {
        ...
        System.arraycopy(other.last, 0, last, 0, other.len);
     ...
     * @return true if there is a prefix
     */
    public boolean startsWith(TagPath prefix) {
        ...
        for (int i = 0; i < prefix.len; i++) {
            if (!tags[i].equals(prefix.tags[i]) || qualifiers[i] != prefix.qualifiers[i] || last[i] != prefix.last[i]) {
                return false;
            }
        }
        // yes
     ...
     * @return true if there is the suffix
     */
    public boolean endsWith(TagPath suffix) {
        ...
        for (int i = suffix.len - 1; i >= 0; i--) {
            if (!tags[i + offset].equals(suffix.tags[i]) || qualifiers[i + offset] != suffix.qualifiers[i] || last[i + offset] != suffix.last[i]) {
                return false;
            }
        }
        // yes
    ...
    public boolean equals(Object obj) {
        ...
        // Elements ?
        for (int i = 0; i < len; i++) {
            if (!tags[i].equals(other.tags[i]) || qualifiers[i] != other.qualifiers[i] || last[i] != other.last[i]) {
                return false;
            }
        }
     ...
     *
     * @param which 0-based number. If negative starts from the end. get(-1)
     * returns the las element (equivalent to getLast)
     ...
     *
     * @return the path as a string to be displayed
     */
     ...
     *
     * @param c collection of tagPath
     * @return array out of collection
     */
     ...
     *
     * @param paths paths
     * @return array of tag Paths out of an array of strings
     */
    ...
    protected boolean recurse(PropertyVisitor visitor, Property prop, String tag, int pos, boolean backtrack) {
        ...
        if (last[pos]) { // if last loop from last to first
        ...
        // if qualifier loop from 0 to last.
