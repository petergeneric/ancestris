 ...
 * <!ATTLIST T ax CDATA>
 * <!ATTLIST T ay CDATA>
 * </pre> wx,wy are weight arguments - gx,gy are grow arguments
 *
 * examples:
...
public class NestedBlockLayout implements LayoutManager2, Cloneable {
    ...
    private class DescriptorHandler extends DefaultHandler {
        ...
        public InputSource resolveEntity(String publicId, String systemId) {
            ...
            // webserver returning a custom 404 someone might read a layout string from getResourceAsStream()
    ...
    private static abstract class Block implements Cloneable {
            ...
            if (pad != null) {
                ...
                switch (pads.length) {
                    case 0:
                        break;
                    case 1:
                        padding.set(Integer.parseInt(pads[0]), Integer.parseInt(pads[0]), Integer.parseInt(pads[0]), Integer.parseInt(pads[0]));
                        break;
                    case 2:
                        padding.set(Integer.parseInt(pads[0]), Integer.parseInt(pads[1]), Integer.parseInt(pads[0]), Integer.parseInt(pads[1]));
                        break;
                    case 3:
                        padding.set(Integer.parseInt(pads[0]), Integer.parseInt(pads[1]), Integer.parseInt(pads[2]), Integer.parseInt(pads[1]));
                        break;
                    case 4:
                        padding.set(Integer.parseInt(pads[0]), Integer.parseInt(pads[1]), Integer.parseInt(pads[2]), Integer.parseInt(pads[3]));
                        break;
                    default:
        ...
        protected abstract void toString(StringBuilder result);
    ...
    private static class Row extends Folder {
        ...
        void layoutFolder(Rectangle in) {
            ...
            double weightFactor = localWeight > 0 ? spare / localWeight : 0;
            int growFactor = weightFactor == 0 && localGrow > 0 ? spare / localGrow : 0;
    ...
    public void addLayoutComponent(Component comp, Object key) {
        ...
        List<Block> path = root.setContent(key, comp, new ArrayList<>());
        if (!path.isEmpty()) {
            components.add(comp);
            return;
        }
