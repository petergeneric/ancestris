...
public final class ZipExplorerSendTranslationAction implements ActionListener {

    private final static String SEND = NbBundle.getMessage(ZipExplorerSendTranslationAction.class, "SendTranslationAction.button.send");
    private Preferences modulePreferences = NbPreferences.forModule(ZipExplorerSendTranslationAction.class);
    File zipOutputFile = null;
    ZipExplorerSendTranslationPanel sendTranslationPanel = new ZipExplorerSendTranslationPanel();
    String archiveName = "";
    String filePath = "";
    String prefix = "";
    String suffix = "";
    ...
    private class SendMessageWorker implements Runnable {
        ...
        public void run() {
            ...
            try {
                ...
                NotifyDescriptor nd = new NotifyDescriptor.Message(NbBundle.getMessage(ZipExplorerSendTranslationAction.class, "SendTranslationAction.msg.thankyou"), NotifyDescriptor.INFORMATION_MESSAGE);
                DialogDisplayer.getDefault().notify(nd);
            } catch (Exception e) {
                response = e.getMessage();
                Logger.getLogger(ZipExplorerSendTranslationAction.class.getPackage().getName()).log(Level.INFO, "{0}", e);
                NotifyDescriptor nd = new NotifyDescriptor.Message(NbBundle.getMessage(ZipExplorerSendTranslationAction.class, "SendTranslationAction.msg.senderror")
    ...
    public void actionPerformed(ActionEvent actionEvent) {
        if (modulePreferences.get("mail.host", "").equals("")) {
            NotifyDescriptor nd = new NotifyDescriptor.Message(NbBundle.getMessage(ZipExplorerSendTranslationPanel.class, "SendTranslationPanel.msg.setParameters"), NotifyDescriptor.INFORMATION_MESSAGE);
            DialogDisplayer.getDefault().notify(nd);
            ...
            if (zipArchive != null) {
                if (zipArchive.write() == true) {
                    ...
                    NotifyDescriptor nd = new NotifyDescriptor.Message(NbBundle.getMessage(ZipExplorerSendTranslationAction.class, "SendTranslationAction.msg.nothingToSend"), NotifyDescriptor.INFORMATION_MESSAGE);
                    DialogDisplayer.getDefault().notify(nd);
                }
            }
        }
    ...
    private void setDefaultValues(ZipExplorerSendTranslationPanel sendTranslationPanel) {
        ...
        String TS = new SimpleDateFormat(NbBundle.getMessage(ZipExplorerSendTranslationAction.class, "SendTranslationAction.msg.subject.date")).format(new Date());
        String subject = "[" + NbBundle.getMessage(ZipExplorerSendTranslationAction.class, "SendTranslationAction.msg.subject.tag", fromLocale, toLocale) + " " + TS + "] ";
        sendTranslationPanel.setSubjectFormattedTextField(subject);
    }

    private void saveValues(ZipExplorerSendTranslationPanel sendTranslationPanel) {
