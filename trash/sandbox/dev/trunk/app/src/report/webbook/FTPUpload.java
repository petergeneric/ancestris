...
public class FTPUpload extends Thread {
     ...
     *  Constructor
     */
    public FTPUpload(ReportWebBook report, String host, String user, String password, List<File> localFiles, String localRoot, String remoteRoot, FTPLog log, FTPProgressStatus progress, FTPRegister uploadRegister) {
        //Set up thread
        super("PutFile");
    ...
    public void run() {
       ...
       if (cpt < totalToTransfer || moreThanOneLoop) {
          log.write(report.translate("upload_error"), true);
          }
       if (cpt == totalToTransfer) {
          log.write(report.translate("upload_alldone"), true);
          }
       progress.close();
       }
    ...
    */
    private synchronized boolean connectToServer() {
       ...
       try  {
          ...
          log.write(report.translate("upload_errorConn", e));
    ...
    public boolean put(File fileToTransfer, String storeName) {
        ...
        Timer timer = new Timer(120000, new ActionListener() {
           public void actionPerformed(ActionEvent e) {
              try {
                 logEvent(report.translate("upload_error_timeout"));
                 abort();
                 timeOutError = true;
        ...
        try {
            ...
            debugMsg(report.translate("upload_uponefile"));
            dataTransferred = 0;
            ...
            //Close connection
            debugMsg(report.translate("upload_storefile"));
            dataOutput.close();
            FTPdata.close();
            ...
            if (cmdResponseCode == TRANSFER_OK) {
               debugMsg(report.translate("upload_success"));
            }
            ...
            timer.stop();
            timeOutError = isTimeout(e);
            return exceptionMsg(e, report.translate("upload_errorPut", e));
        }
        return true;
    }
    ...
    */
    ...
    private synchronized boolean closeServerConnection() {
        ...
        if (FTPcmd != null) {
            try {
                ...
                log.write(report.translate("upload_errorClose", e));
               }
           }
    ...
    private boolean calculateWhatToDo(boolean moreThanOneLoop) {
       ...
       try  {
          logEventTitle(report.translate("upload_check"));
          uploadRegister.calculate(report.getUploadType());
          uploadRegister.save();
          if (!moreThanOneLoop) {
             totalToTransfer = uploadRegister.getNbFilesToTransfer();
             }
          logEventTitle(report.translate("upload_foundlocal", localFiles.size()));
          logEventTitle(report.translate("upload_foundtrsf", totalToTransfer));
          }
       catch (Exception e) {
          exceptionMsg(e, report.translate("upload_error_check", e));
          return false;
          }
       return true; 
       }
    ...
    private boolean uploadLocalFilesToServer() {
       ...
       logEventTitle(report.translate("upload_starting"));
       try  {
            for (Iterator it = localFiles.iterator(); it.hasNext();) {
               ...
               if (!uploadRegister.isToTransfer(file)) {
                  if (!moreThanOneLoop) {
                     log.write(report.translate("upload_noneed", new String[] { currentLocalDir, file.getName() } ));
                     }
                  continue;
                  }
               ...
               if (currentLocalDir.compareTo(currentRemoteDir) != 0) {
                 ...
                 if (!cd(remoteRoot)) { 
                    logEvent(report.translate("upload_dirnotthere"));
                    logEvent("mkdir "+remoteRoot);
                    mkdir(remoteRoot);
                    logEvent("cd "+remoteRoot);
                    if (!cd(remoteRoot)) { 
                       logEvent(report.translate("upload_error_ftp_cd", cmdResponse));
                       return false;
                       }
                    }
               ...
               if (!put(file, storeName)) { 
                  logEvent(report.translate("upload_error_ftp_put", "PUT"));
                  return false; 
                  }
               cpt++;
               uploadRegister.setFileTransferred(file);
               totalSize += (dataTransferred / 1024);
               log.write(report.translate("upload_transferred", new String[] { currentLocalDir, file.getName(), String.valueOf(dataTransferred) } ));
               logEventTitle(report.translate("upload_donesofar", new String[] { String.valueOf(cpt), String.valueOf(totalToTransfer), String.valueOf(totalSize) } ));
          ...
          logEvent(report.translate("upload_trsfComplete"));
          uploadRegister.save();

          } catch(IOException e) {
             exceptionMsg(e, report.translate("upload_errorLoop", e));
             return false;
          }
    ...
    private boolean removeServerFiles() {
       ...
       logEventTitle(report.translate("upload_remove", totalToRemove));
       int FTP_SYNCHRONISE = 2;
       if (totalToRemove == 0) {
          logEventTitle(report.translate("upload_noremove"));
          return false;
          }
       if (report.getUploadType() != FTP_SYNCHRONISE) {
          logEventTitle(report.translate("upload_usernoremove"));
          return false;
          }

       progress.setTotal(totalToRemove);
       logEventTitle(report.translate("upload_startingrm"));
       try  {
            ...
            for (Iterator it = listToRemove.iterator(); it.hasNext();) {
               ...
               if (remoteDir.compareTo(currentRemoteDir) != 0) {
                 ...
                 if (!cd(remoteDir)) { 
                    logEvent(report.translate("upload_nodir"));
               ...
               logEvent("rm "+ file);
               if (!rm(file)) { 
                  logEvent(report.translate("upload_cannotrm", file));
                  }
               else {
                  logEvent(report.translate("upload_removed", file));
                  }
               uploadRegister.setFileRemoved(key);
               progress.increment(1);
            }

          logEvent(report.translate("upload_rmComplete"));
          uploadRegister.save();

          } catch(IOException e) {
             exceptionMsg(e, report.translate("upload_errorrm", e));
             return false;
          }
    ...
    */
    ...
    private boolean exceptionMsg(Exception exception, String msg) {
        logEvent(msg);
        progress.setButton(report.translate("upload_button_term"));
