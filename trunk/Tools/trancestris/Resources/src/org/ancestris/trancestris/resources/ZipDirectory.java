...
//                zipDirectory.putRef(tokenizefilePath, zipEntry, inputstream, filePath);
    ...
    public ArrayList<ZipDirectory> getDirs() {
        List<ZipDirectory> list = new ArrayList<>(dirs.values());
        Collections.sort(list, new Comparator<ZipDirectory>() {
            public int compare(ZipDirectory o1, ZipDirectory o2) {
    ...
    public boolean cleanTranslation() {
        for (ZipDirectory zipDirectory : dirs.values()) {
            if (zipDirectory.cleanTranslation() == true) {
                return true;
            }
        }
        if (resourceFile != null) {
            return resourceFile.cleanTranslation();
        } else {
            return false;
        }
    }
