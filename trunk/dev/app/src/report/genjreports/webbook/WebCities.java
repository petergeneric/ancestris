...
public class WebCities {
   ...
   * Constructor
   */
  public WebCities(ReportWebBook report, Gedcom gedcom, Indi indiDeCujus, WebSection section) {
     this.report = report;
     this.gedcom = gedcom;
  ...
  public void run() {
    ...
    sectionList = report.getSection("Citiesdetails");

    // HEAD
    wh.printOpenHTML(out, "Citieslist", section);
  ...
  private void exportData(File dir, PrintWriter out) {
    ...
    Iterator it = wh.getCities(gedcom).iterator();
    char last = ' ';
    int cpt = 1, iNames = 1;
