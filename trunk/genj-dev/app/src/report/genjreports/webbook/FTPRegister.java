...
public class FTPRegister {
  ...
  private final static int
    ...
    FTP_INCREMENTAL = 1,
    FTP_SYNCHRONISE = 2;
  private int uploadType = FTP_INCREMENTAL;
   ...
   * Constructor
   */
  public FTPRegister(String filename, String host, String targetdir, File localRoot, int uploadType, boolean reset) {
  ...
  public boolean calculate(int uploadType) {
    ...
    for (Iterator it = properties.keySet().iterator(); it.hasNext(); ) {
      ...
      if (uploadType == FTP_SYSTEMATIC) {
         fields[REG_TOBETRSF] = YES;
         nbFilesToTransfer++;
         }

      if ((uploadType == FTP_INCREMENTAL) && (!equals(fields[REG_REMOTEMD5], fields[REG_LOCALMD5])) && (equals(fields[REG_GENLASTRUN], YES))) {
         fields[REG_TOBETRSF] = YES;
         nbFilesToTransfer++;
         }

      if ((uploadType == FTP_SYNCHRONISE) && (!equals(fields[REG_REMOTEMD5], fields[REG_LOCALMD5])) && (equals(fields[REG_GENLASTRUN], YES))) {
         fields[REG_TOBETRSF] = YES;
         nbFilesToTransfer++;
         }

      // if option = 3 and generated = 0 and md5 remote <> 0 then set toBeDeleted flag to 1
      if ((uploadType == FTP_SYNCHRONISE) && (!equals(fields[REG_REMOTEMD5], "")) && (!equals(fields[REG_GENLASTRUN], YES))) {
