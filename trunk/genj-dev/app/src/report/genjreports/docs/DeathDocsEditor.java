...
import genjreports.docs.panels.IndiDeathPanel;
...
/**
 * The editor for entering a death document
 */
class DeathDocsEditor extends EditorDocs implements DocsListener {
  ...
  private IndiDeathPanel indiPanel;
  ...
  /** number of witnesses for a death */
  ...
  public String getTitle() {
     return view.translate("DeathDocsUpper");
  }

  public String getSmallTitle() {
     return view.translate("DeathDocsLower");
  }
  ...
  private class OK extends Action2 {
    ...
    protected void execute() {
      HelperDocs.processOk(gedcom, view, registry, HelperDocs.DEAT_EDITOR);
      }
    }
  ...
  private class Cancel extends Action2 {
    ...
    /** cancel current proxy */
    protected void execute() {
      HelperDocs.processCancel(gedcom, view, registry, HelperDocs.DEAT_EDITOR);
      }
  }
  ...
  /**
   * Populate panel with death document format
   */
  public void populatePanel(JPanel panel) {
    ...
    if (!tabbed) {
       ...
       jp2.add(witnessesPanel);
       tabbedPane.add(view.translate("Declarant"), jp2);

       JPanel jp3 = new JPanel(fl);
       jp3.add(indiPanel);
       tabbedPane.add(view.translate("Deceived"), jp3);
  ...
  public Entity updateGedcom() throws GedcomException {
   ...
   // deceased is mandatory
   if (!indiPanel.isOK(1)) throw new GedcomException(view.translate("msg_missingDead"));

   // Update or create deceased person
   Indi deceased = indiPanel.upcreateIndi(1, datePanel.refLISTD, pitDocDate, datePanel.refPLACD, datePanel.refDATED, datePanel.refAGNCD.getText());

   // Update or create spouse
   Indi spouse = null;
   if (indiPanel.isOK(2)) spouse = indiPanel.upcreateIndi(2, null, pitDocDate, null, null, null);

   // Update or create family made of deceased and spouse
   HelperDocs.upcreateFamily(gedcom, deceased, spouse);

   // Update or create father
   Indi father = null;
   if (indiPanel.isOK(3)) father = indiPanel.upcreateIndi(3, null, pitDocDate, null, null, null);

   // Update or create mother
   Indi mother = null;
   if (indiPanel.isOK(4)) mother = indiPanel.upcreateIndi(4, null, pitDocDate, null, null, null);

   // Update or create family made of father and mother and linking deceased in
   HelperDocs.upcreateParents(gedcom, deceased, father, mother);

   // Update or Create repository and Source (skip if empty)
   actPanel.upcreateAct(deceased.getProperty("DEAT"));

   // Update or Create witnesses (if required and not empty, as notes or individuals; skip if empty)
   witnessesPanel.upcreateWitnesses(deceased.getProperty("DEAT"), pitDocDate);
  ...
  private class CopiedDocument {
    ...
    public Indi spouse = null;
    public Indi father = null;
    public Indi mother = null;
    public Fam familyIndi = null;
    public Fam familyParents = null;
    public Indi[] witnesses = new Indi[NB_WIT];
    public Entity source = null;
    public Entity repo = null;
    public String invalidMsg = view.translate("msg_invalidDeathCopy");

    /** constructor */
    private CopiedDocument() {

       indi = datePanel.getIndi();
       if (indi != null) listEnt.add(indi);
       spouse = indiPanel.getIndi(indiPanel.refSINDID);
       if (spouse != null) listEnt.add(spouse);
       ...
       familyIndi = HelperDocs.getFamily(indi, spouse);
       if (familyIndi != null) listEnt.add(familyIndi);
       familyParents = HelperDocs.getFamily(father, mother);
       if (familyParents != null) listEnt.add(familyParents);
  ...
  public void populateAll(Entity ent) {
    ...
    // Set witnesses
    witnessesPanel.setWitnesses(deceased);

    // Get parents and spouse of child and display them
    if (deceased != null) {
       Fam parents = deceased.getFamilyWhereBiologicalChild();
       Indi father = parents != null ? parents.getHusband() : null;
       Indi mother = parents != null ? parents.getWife() : null;
       Indi spouse = getLastSpouse(deceased);
       indiPanel.setIndi(deceased, father, mother, spouse);
       }
    else {
       indiPanel.setIndi(null, null, null, null);
       }

    // Get source and display it
    actPanel.setSource(deceased);
   ...
   * get Last Spouse
   */
  public Indi getLastSpouse(Indi indi) {
    ...
    if (fams == null || fams.length == 0) return null;

    spouse = fams[fams.length -1].getOtherSpouse(indi);
