...
public class BlueprintRenderer {
  ...
  private class MyHTMLFactory extends HTMLFactory {
    ...
    public View create(Element elem) {
      ...
      if ("prop".equals(name)) {
        final PropertyView result = new PropertyView(elem);
        volatileViews.add(result);
        return result;
      }
      
      // check the IFValue tag
      if ("ifvalue".equals(name)) {
          final IfValueView result = new IfValueView(elem);
          volatileViews.add(result);
          return result;
      }
      ...
      if ("media".equals(name)) {
        final MediaView result = new MediaView(elem);
        volatileViews.add(result);
        return result;
      }
  ...
  private class IfValueView extends MyView {
    ...
    IfValueView(Element elem) {
      ...
      if (ep!=null) try {
        ...
        if (LOG.isLoggable(Level.FINER))
          LOG.log(Level.FINER, "got wrong path "+ep);
      }
     ...
     * Get Property
     */
    private Property getProperty(TagPath myPath) {
      // still looking for property?
      if (cachedProperty!=null)
        return cachedProperty;
      if (entity==null||myPath==null)
        return null;
      cachedProperty = BlueprintRenderer.this.getProperty(entity, myPath);
      return cachedProperty;
    }
